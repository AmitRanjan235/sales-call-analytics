...
class AIInsightModule:
    ...
    def calculate_agent_talk_ratio(self, transcript: str) -> float:
        ...
        try:
            ...
            for line in lines:
                ...
                is_customer = any(re.match(pattern, line) for pattern in customer_patterns)
                
                # Remove speaker labels and count words
                clean_line = re.sub(r'^(Agent|A|Customer|C|AGENT|CUSTOMER|agent|customer):\s*', '', line)
    ...
    def find_similar_calls(self, target_embedding: List[float], 
    ...
    def generate_coaching_nudges(self, transcript: str, 
                               sentiment_score: float, 
                               talk_ratio: float) -> List[str]:
        ...
        try:
            if self.openai_client:
                ...
                try:
                    ...
                    nudges = [line.strip() for line in content.split('\n') if line.strip()][:3]
                except Exception as e:
            ...
            return ["Focus on active listening", "Ask more open questions", "Empathize with customer needs"]

    def _generate_rule_based_nudges(self, sentiment_score: float, talk_ratio: float) -> List[str]:
