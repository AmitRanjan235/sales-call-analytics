...
def recalculate_analytics():
    ...
    try:
        ...
        for (agent_id,) in agents:
            ...
            if calls:
                # Calculate averages
                sentiment_scores = [call.customer_sentiment_score for call in calls if call.customer_sentiment_score is not None]
                talk_ratios = [call.agent_talk_ratio for call in calls if call.agent_talk_ratio is not None]
        ...
        db.commit()
        logger.info(f"Analytics recalculation complete. Updated {updated_count} agents.")
