...
SAMPLE_DATA = {
    "business_problem": ["managing customer data", "tracking sales pipeline", "automating workflows", "improving team communication"],
    "product_name": ["SalesFlow Pro", "DataSync Plus", "WorkflowMaster", "TeamConnect"],
    "competitor": ["Salesforce", "HubSpot", "Pipedrive", "Monday.com"],
    "unique_value_prop": ["advanced AI analytics", "seamless integrations", "intuitive interface", "comprehensive reporting"],
    "timeline": ["Q2 next year", "within 3 months", "as soon as possible", "by end of year"],
    "closing_response": ["Sounds good!", "I need to discuss with my team first.", "Let me think about it.", "Yes, let's move forward."],
    "product_feature": ["reporting dashboard", "data export", "user permissions", "mobile app"],
    "problem_duration": ["happening since yesterday", "going on for a week", "started this morning", "occurring intermittently"],
    "action": ["generate reports", "export data", "log in", "sync data"],
    "problem_description": ["I get an error message", "the page won't load", "data isn't syncing", "nothing happens when I click"],
    "solution_step_1": ["clear your browser cache", "check your permissions", "refresh the data", "restart the application"],
    "solution_step_2": ["try the action again", "contact admin if issues persist", "wait 5 minutes for sync", "check the updated settings"],
    "additional_request": ["No, that's all. Thanks!", "Can you show me how to do X?", "What about feature Y?", "I have another question."],
    "initial_response": ["I'm still interested", "I have some concerns", "My team needs more info", "We're comparing options"],
    "concern": ["price", "implementation time", "training requirements", "integration complexity"],
    "benefit_1": ["30% time savings", "improved accuracy", "better collaboration", "increased sales"],
    "benefit_2": ["ROI of 200%", "90% user adoption", "50% cost reduction", "doubled productivity"],
    "timeframe": ["one week", "two weeks", "a few days", "less than a week"],
    "price_objection": ["It seems expensive", "Budget is tight", "Need to justify cost", "Comparing with cheaper options"],
    "roi_timeframe": ["6 months", "one year", "3 months", "9 months"],
    "final_response": ["Let me discuss with my team", "I'm ready to move forward", "Send me the proposal", "I need more time to decide"]
}
...
async def update_analytics(db: Session):
    ...
    try:
        ...
        for (agent_id,) in agents:
            ...
            if calls:
                avg_sentiment = sum(call.customer_sentiment_score or 0 for call in calls) / len(calls)
                avg_talk_ratio = sum(call.agent_talk_ratio or 0 for call in calls) / len(calls)
                total_calls = len(calls)
                
                # Update or create analytics record
                analytics = db.query(Analytics).filter(Analytics.agent_id == agent_id).first()
